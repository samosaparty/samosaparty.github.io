...
import { Laptop, Search, AlertCircle, Download, User } from 'lucide-react';
        ...
        <div>
          <h1 className="text-2xl font-extrabold flex items-center gap-3 tracking-tight" style={{ color: 'var(--text-main)' }}>
            <div className="p-2.5 rounded-xl" style={{ backgroundColor: 'var(--secondary)', color: 'var(--primary)' }}>
              <Laptop className="w-6 h-6" strokeWidth={2.5} />
            </div>
            Corporate Laptop
          </h1>
          <p className="text-sm mt-2 font-medium" style={{ color: 'var(--text-muted)' }}>
            Corporate Laptop directory and information.
          </p>
        </div>
        ...
        <div className="p-8 md:p-10 relative min-h-[400px]">
          ...
          {loading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 z-10 backdrop-blur-sm">
              ...
              <p className="text-slate-500 font-medium">Loading Corporate Laptop data...</p>
            </div>
          ) : null}
          ...
          <table>
            <thead>
              <tr>
                {columns.map((col, idx) => (
                  <th key={idx} title={col.key}>{col.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {!loading && filteredData.length === 0 ? (
                <tr>
                  <td colSpan={Math.max(1, columns.length)} className="px-6 py-20 text-center">
